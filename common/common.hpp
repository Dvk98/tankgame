...
struct Line {
    constexpr Line(f32 m, f32 b)
        : m(m)
        , b(b) {
    };
    constexpr Line(f32 m, f32 b, f32 h, f32 v)
        : m(m)
        , b(b)
        , v(v)
        , h(h) {
    ...
    Vec2 GetIntersection(Line line) {
        f32 x = (line.b - this->b) / (this->m - line.m);
        return Vec2{x, this->m * x + this->b};
    }
};
