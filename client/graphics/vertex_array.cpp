...
void VertexArray::CreateUnitQuad() {
    // This is used for instanced rendering of quads.
    // It represents just a quad that uses the whole texture, and has a destination rectangle
    // that starts at (0|0) and has size (1|1), that will then be translated and scaled + rotated by the
    // model matrix of the instance.
    this->Add(Vec2{0.0f}, Vec2{1.0f}, Vec2{0.0f}, Vec2{1.0f}, Color{255, 255, 255, 255});
}

void VertexArray::AddPoint(Vec2 pos, Color color) {
    Vertex vertex;
    vertex.position = pos;
...
void VertexArray::AddTriangle(const Vertex vertices[3]) {
    this->dirty = true;
    this->num_vertices = 0;
    this->vertices.insert(this->vertices.end(), vertices, vertices + 3);
}
